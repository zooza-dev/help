...
AREA_HINTS = [   # (regex on heading+alt+filename+article, screen id) — coarse placement when no cue matches
    ...
    (re.compile(r"\brefunds? (issued|list)\b|\bissued refunds\b", re.I), "payments.refunds.issued"),
    (re.compile(r"\bdirect debit mandates?\b|\bmandates? (list|page)\b", re.I), "payments.mandates"),
    (re.compile(r"\bimport payments?\b|\bpayment import\b|\bcsv-payment-import\b", re.I), "payments.transactions.import"),
    (re.compile(r"\btransactions?\b.*\b(list|page|tab|export)\b|\btransactions-", re.I), "payments.transactions.cash_transfer"),
    (re.compile(r"\bscheduled payments? overview\b|\binstallments? overview\b", re.I), "payments.scheduled"),
    (re.compile(r"\bdiscount codes?\b|\bdiscounts? (list|page)\b|\bdiscount-code", re.I), "payments.discounts"),
    (re.compile(r"\bloyalty\b", re.I), "payments.loyalty"),
    (re.compile(r"\binvoices? (list|page)\b|\binvoices-list\b", re.I), "payments.invoices"),
    (re.compile(r"\bpayments? (overview|dashboard|page)\b|\bpayments-overview\b", re.I), "payments.overview"),
    (re.compile(r"\bsend message\b|\bcompose\b|\bcommunication-send", re.I), "communication.compose"),
    (re.compile(r"\bmessage jobs?\b", re.I), "communication.message_jobs"),
    (re.compile(r"\bmessage templates?\b|\btemplates? (list|editor|page)\b|\bmessage-templates\b", re.I), "communication.templates"),
    (re.compile(r"\bwhatsapp\b", re.I), "communication.whatsapp"),
    (re.compile(r"\bsent (emails?|messages?|communication)\b|\bcommunication history\b", re.I), "communication.history"),
    (re.compile(r"\bdaily overview\b", re.I), "calendar.daily"),
    (re.compile(r"\bplanning calendar\b|\bplanner\b", re.I), "calendar.planner"),
    (re.compile(r"\bcalendar\b", re.I), "calendar.week"),
    (re.compile(r"\bNPS\b", re.I), "feedback.nps"),
    (re.compile(r"\bgoogle reviews?\b|\breviews? (list|page)\b", re.I), "feedback.reviews"),
    (re.compile(r"\bsubmissions?\b", re.I), "feedback.submissions"),
    (re.compile(r"\bsystem messages\b", re.I), "reports.system_messages"),
    (re.compile(r"\bsession notifications?\b", re.I), "reports.event_notifications"),
    (re.compile(r"\boutstanding payments\b", re.I), "reports.outstanding_payments"),
    (re.compile(r"\btrial sessions? report\b|\breports-trial", re.I), "reports.trials"),
    (re.compile(r"\breports?[- ]dashboard\b|\bnick'?s dashboard\b|\bdashboard\b", re.I), "reports.dashboard"),
    (re.compile(r"\breports?\b", re.I), "reports.registrations"),
    (re.compile(r"\binstructor (detail|profile|settings|availability)\b|\btrainer-", re.I), "team.trainer.detail"),
    (re.compile(r"\binstructors? (list|page)\b|\bmanaging-instructors\b", re.I), "team.trainers"),
    (re.compile(r"\bpublish\b|\bwidgets?\b|\bbooking form\b", re.I), "team.publish"),
    (re.compile(r"\bintegrations?\b|\bstripe\b|\bgocardless\b|\bxero\b|\bfastpay\b|\bvimeo\b|\bslack\b|\bpower bi\b", re.I), "team.integrations"),
    (re.compile(r"\bzooza ai\b|\bmcp\b|\bclaude\b", re.I), "team.mcp"),
    (re.compile(r"\bproducts?\b|\bentry pass\b|\bvoucher\b", re.I), "products.list"),
    (re.compile(r"\bservices?\b", re.I), "services.list"),
