...
def _make_docusaurus_config(staging: bool = False) -> str:
    sitemap_block = "" if staging else """\
        sitemap: {
          changefreq: 'weekly',
          priority: 0.7,
          ignorePatterns: ['/tags/**'],
...
"""
    robots_content = "noindex, nofollow" if staging else "index, follow"
    gtag_plugin = "" if staging else """\
...
def _write_docusaurus_config(out: Path, staging: bool = False) -> None:
    (out / "docusaurus.config.js").write_text(_make_docusaurus_config(staging=staging), encoding="utf-8")
...
def _write_custom_css(out: Path) -> None:
    ...
    # Docusaurus v3 requires directory form: src/theme/NotFound/index.js
    (out / "src" / "theme" / "NotFound").mkdir(parents=True, exist_ok=True)
    (out / "src" / "theme" / "NotFound" / "index.js").write_text(_404_JS, encoding="utf-8")
...
def _write_static_files(out: Path, staging: bool = False) -> None:
    ...
    if staging:
        robots_content = (
            "# Staging environment — not for public indexing\n"
            "User-agent: *\n"
            "Disallow: /\n"
        )
    else:
...
def build_docusaurus(dry_run: bool = False, clean: bool = False, staging: bool = False) -> dict:
    ...
    _write_docusaurus_config(OUTPUT_DIR, staging=staging)
...
def main() -> None:
    ...
    stats = build_docusaurus(dry_run=dry_run, clean=clean, staging=staging)
