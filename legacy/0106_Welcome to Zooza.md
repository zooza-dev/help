...
**URL:** https://support.zooza.online/portal/en/kb/articles/power-bi-data-description
...
To assemble a complete view of each participant’s journey, join Attendance with table Registrations, Schedules, Courses or Events.
...
Boolean flag 0/1 whether the course uses additional fields
...
Session status (scheduled/deleted/unplanned)

Date & Time

Location

Instructor details including payroll settings

Static session details - any additional settings visible in the session’s detail view, such as course, class, session name, description, capacity, etc.

Attendance metrics (calculated number of participants, number of cancellations, waitlists,...)
...
We recommend joining it with a schedules table with __calc__public__labels.
...
Class Details: name, date & time, duration, instructor, and location

Capacity & Registration: class capacity, total registrations, trial registrations, and waitlist count

Pricing & Payments: course price, payment settings, and debt summary (total issued, paid, and outstanding balance)

All settings visible in settings tile in Zooza (online registration, class properties, etc.)

Performance Metrics: average feedback score and session count

Instructor details including rates

Tip

: Because it aggregates both static settings and dynamic metrics, you can join Schedules with any related table—by schedule_id, course_id, instructor_id, or place_id—to build a complete view.
...
Tip:
 ...
 Join on trainer_rate_type and trainer_id with the Trainer Rates Types and Schedules tables.
