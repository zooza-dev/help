...
Yes. Open the class and click **Add booking**, or go through the booking form on behalf of a parent. The recommended flow is still for parents to book online themselves; a manual booking does not take the client through the online payment step, so they pay later from their client profile or you record the payment yourself.

Trial bookings can be created by an admin too: create the booking, open it, click **Change status** and use **Convert to trial** (shown when the class supports trials), then pick the session(s). See [Can an admin manually create a trial booking?](trials-faq.md#can-an-admin-manually-create-a-trial-booking).
