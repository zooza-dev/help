...
Create an ignore filter. Either trigger it by ignoring one payment and clicking "Create filter", or create it manually in **Sales & Payments → Payment reconciliation → AI rules & filters**. Use the exact payer IBAN plus a note pattern if needed.
